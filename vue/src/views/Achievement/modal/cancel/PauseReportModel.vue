<template>
    <ModalLayout :show="show" title="中断の報告文書" maxWidth="max-w-2xl" @close="onClose()">
        <div class="px-2">
            <div class="my-2 form-border preview" ref="body_text">
                <table>
                    ...
                    <tr>
                        <td>中止理由</td>
                        <td>: {{ cancel.pause_finish_reason ?? '未記載' }}</td>
                    </tr>
                </table>
                <div class="d-grid">
                    <span>＊ポンプ回収用梱包資材の希望　記載なし　</span>
                    <span>★治療中断通知連絡メールは、届いておりませんが</span>
                    <span>川野様より下記のメールが届いておします。</span>
                    <span>いつも大変お世話になっております。</span>
                    <span>下記患者さんの治療中止の一報が入りましたので連絡します。</span>
                    <span>ポンプレンタル中止連絡、現物返送は依頼済みです。</span>
...
export default {
    ...
    created: function () {
        if(this.cancel.pause_start_date) {
            this.pause_start_date = this.cancel.pause_start_date
            if(typeof this.cancel.pause_start_date !== 'object') this.pause_start_date = new Date(this.cancel.pause_start_date)
            this.pause_start_date = moment(this.pause_start_date).format('YYYY年MM月DD日')
        }
    }
}
</script>