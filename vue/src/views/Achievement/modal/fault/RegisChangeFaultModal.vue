<template>
    <ModalLayout :show="show" title="故障交換登録" maxWidth="max-w-sm" @close="onClose()">
        <div class="p-2">
        ...
        <div class="pt-2 pb-3 d-flex justify-content-around">
            ...
            <button class="btn w-[6rem]" @click="createChangeFault()" :disabled="!payload.serial_no && !payload.patient_cd">作成</button>
        </div>
    </ModalLayout>
</template>
<style scoped>
...
export default {
    ...
    methods: {
        ...
        createChangeFault: async function () {
            await this.storeChangeFault(this.payload).then((res) => {
                if(res && res.code == 200) {
                    this.reload  = true
                    this.onClose()
                }
