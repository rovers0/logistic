<template>
    <div class="p-2 wrapper-scroll transit">
        <form class="gap-5 mb-2 d-flex" @submit.prevent="submitSearch()">
            <div class="flex-1 gap-3 d-flex">
                ...
                <div class="d-flex align-items-center basis-[25%]">
                    <label class="w-[5rem] label-text">処理状況</label>
                    <select class="flex-1 form-select" v-model="search.status">
                        <option :value="null">全て</option>
                        <template v-for="st in master.exchange_status">
                            <option v-if="search.is_complete || st.id != 15" :value="st.id">{{ st.value }}</option>
                        </template>
                    </select>
                </div>
                <div class="d-flex align-items-center basis-[25%]">
                    <label class="w-[4rem] label-text">予定日</label>
                    <select class="flex-1 form-select" v-model="search.plan">
                        <option :value="null">全て</option>
                        <option v-for="st in master.exchange_plan" :value="st.id">{{ st.value }}</option>
                    </select>
                </div>
                <div class="d-flex align-items-center">
                    <Datepicker class="w-[10rem]" v-model="search.plan_date" format="yyyy/MM/dd(E)"></Datepicker>
                </div>
                <label class="gap-2 label-text fw-bold d-flex align-items-center">
                    <input type="checkbox" class="mt-0 form-check-input" v-model="search.is_complete">旧ポンプ回収完了を含む
                </label>
            </div>

            <div class="gap-2 d-flex align-items-center">
                <button type="button" class="btn default w-[7rem]" @click="resetSearch(); submitSearch();">リセット</button>
                <button type="submit" class="btn w-[7rem]">検索</button>
            </div>
        </form>

        <div class="w-[90%] mx-auto parent-scroll mb-3" style="max-height: 14rem;">
            <table class="view-scroll t-border">
                ...
                <tbody>
                    <tr v-for="plan in planReport" :key="plan.date">
                        <td class="text-center">{{ displayDate(plan.date) }}</td>
                        <td class="text-center">{{ plan.plan_make_set }} 件</td>
                        <td class="text-center">{{ plan.plan_send_ms }} 件</td>
                        <td class="text-center">{{ plan.plan_send_fac }} 件</td>
                        <td class="text-center">{{ plan.plan_recv_exp }} 件</td>
                        <td class="text-center">{{ plan.plan_send_deli }} 件</td>
                        <td class="text-center">{{ plan.plan_inst_ymc }} 件</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="mb-1 d-flex align-items-center">
            <div class="gap-2 col-4 d-flex align-items-center">
                ...
                <button type="button" class="btn" @click="exportExchangePDF()" :disabled="ids.length == 0">案内書出力</button>
            </div>
            ...
            <div class="col-4 d-flex align-items-center justify-content-end">
                <button type="button" class="btn selection" @click="register()">定期交換データ作成</button>
            </div>
        </div>
        <div class="mb-1 d-flex align-items-center">
            <div class="gap-2 col-4 d-flex align-items-center">
            ...
            <div class="gap-2 col-4 d-flex align-items-center justify-content-end">
                <button type="button" class="btn" @click="openShippingModal()" :disabled="ids.length <= 0">出荷データ作成・指示</button>
                <button type="button" class="btn" @click="exportExchangeExcel()" :disabled="exchangeList.length <= 0">一覧出力</button>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        ...mapState({
            ...
            search: state => state.exchange.search,
        }),
    },
    watch:{
    ...
    methods: {
        ...mapActions('exchange', ['getMasterExchange', 'storeExchange', 'getIndexExchange', 'updatePlan', 'getPlanReportExchange', 'exportPDF', 'exportExcel']),
        ...mapMutations('exchange', ['resetSearch']),
        ...
        register: async function () {
            const ok = await this.$refs.confirmDialog.show({
                title: jp.confirm,
                message: jp.confirm_regis_exchange
            })
            if(ok) {
        ...
        exportExchangeExcel: async function () {
            await this.exportExcel(formatBeforeRequest(this.search))
        }
    },
    created: async function () {
        this.getMasterExchange()
