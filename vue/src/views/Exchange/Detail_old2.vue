    ...
    <div class="p-1 transit wrapper-scroll">
                ...
                <div class="flex-1 d-grid input-gap">
                    ...
                    <div class="gap-3 d-flex">
                        ...
                        <div class="d-flex align-items-center basis-[11%]">
                            <select class="flex-1 form-select" v-model="exchangeDetail.destination.des_title" disabled>
            ...
            <div class="w-full gap-3 d-flex">
                ...
                <div class="d-grid gap-3 basis-[50%]" style="height: fit-content;">
                    ...
                    <div class="d-grid input-gap">
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <!-- <div class="gap-2 d-flex align-items-center">
                                <label class="label-text">交換納品日</label>
                                <Datepicker class="w-[9rem]" v-model="day"></Datepicker>
                            </div> -->
                        ...
                        <div class="gap-3 d-flex justify-content-between">
                            <div class="d-flex align-items-center basis-[40%]">
                                <label class="label-text w-[7rem]">指示NO.</label>
                                <input type="text" class="flex-1 text-center form-control" :value="exchangeDetail.shipment.inst_id ?? '-'" disabled>
                            </div>
                            <div class="gap-2 d-flex align-items-center text-end">
                                <label class="label-text">出荷指示送信日</label>
                                <Datepicker class="w-[9rem]" v-model="exchangeDetail.shipment.send_date" :disabled="checkDisableByStatus('shipping_instruction')"></Datepicker>
                            </div>
                        </div>
                        ...
                        <div class="gap-2 d-flex">
                            <div class="flex-1 d-flex flex-column input-gap">
                                <div class="d-flex align-items-center">
                                    <label class="label-text w-[7rem]">届け先住所</label>
                                    <input type="text" class="form-control text-center me-2 w-[9rem]" v-model="exchangeDetail.shipment.des_postcode" maxlength="8" :disabled="checkDisableByStatus('shipping_instruction')">
                            ...
                            <button class="btn default w-[7rem]" @click="openShippingModal()" :disabled="checkDisableByStatus('shipping_instruction')">出荷指示<br>情報作成</button>
                        </div>

                        <div class="gap-3 d-flex">
                            <div class="flex-1 gap-2 d-flex align-items-center justify-content-end">
                                <label class="label-text">報告書送信日</label>
                                <Datepicker class="w-[9rem]" v-model="exchangeDetail.send_report_date" :disabled="checkDisableByStatus('shipping_instruction')"></Datepicker>
                            </div>
                            <a class="label-text" href="javascript:void(0)" @click="createStatement()" :class="{'disabled': checkDisableByStatus('shipping_instruction')}">メーカー報告文書</a>
                        </div>

                        <!-- group3-row4 -->
                        <div class="d-flex">
                            <textarea class="form-control" v-model="exchangeDetail.shipment.remarks" :disabled="checkDisableByStatus('shipping_instruction')"></textarea>
                        </div>

                        <!-- group3-row5 -->
                        <div class="d-flex align-items-center">
                            <label class="w-[7rem]">メーカー報告I</label>
                            <input type="text" class="flex-1 form-control" :disabled="checkDisableByStatus('shipping_instruction') || true">
                            <!-- <textarea class="flex-1 form-control"></textarea> -->
                        </div>
                    </div>
                    ...
                    <div class="d-grid input-gap">
                        <!-- group4-title -->
                        <div class="d-flex align-items-center justify-content-between">
                            <label class="fw-bold label-text">■ 出荷実績情報</label>
                            <div class="gap-3 d-flex">
                                <a class="label-text" href="javascript:void(0)" :class="{'disabled': checkDisableByStatus('shipping_performance') || true}">配達追跡ページ</a>
                            </div>
                        ...
                        <div class="gap-3 d-flex">
                            <div class="d-flex align-items-center basis-1/3">
                                <label class="label-text w-[7rem]">問合せ番号</label>
                                <input type="text" class="flex-1 text-center form-control" v-model="exchangeDetail.shipment.ship_qacode" :disabled="checkDisableByStatus('shipping_performance')">
                            </div>
                            <div class="d-flex align-items-center basis-1/3">
                                <label class="label-text w-[6rem]">配達完了日</label>
                                <Datepicker class="flex-1" v-model="exchangeDetail.shipment.delivery_date" :disabled="checkDisableByStatus('shipping_performance')"></Datepicker>
                            </div>
                            <div class="d-flex align-items-center basis-1/3">
                                <label class="label-text w-[6rem]">シリアルNo.</label>
                                <input type="text" class="flex-1 text-center form-control" v-model="exchangeDetail.shipment.serial_no" :disabled="checkDisableByStatus('shipping_performance')">
                            </div>
                        </div>

                        <!-- group4-row2 -->
                        <div class="d-flex">
                            <label class="w-[7rem]">メーカー報告Ⅱ</label>
                            <textarea class="flex-1 form-control" disabled>?</textarea>
                        </div>
                    </div>
                    ...
                    <div class="gap-3 d-flex">
                            ...
                            <table class="t-border fit">
                                <tbody>
                                    <tr class="text-center bt-table">
                                        <td>受信日</td>
                                        <td>
                                            <Datepicker class="flex-1 mx-auto" v-model="exchangeDetail.agr_receive_date" :disabled="checkDisableByStatus('consent')"></Datepicker>
                                        </td>
                                    </tr>
                                    <tr class="text-center">
                                        <td>署名日</td>
                                        <td>
                                            <Datepicker class="flex-1 mx-auto" v-model="exchangeDetail.agr_signed_date" :disabled="checkDisableByStatus('consent')"></Datepicker>
                                        </td>
                                    </tr>
                                    <tr class="text-center">
                                        <td>使用開始日</td>
                                        <td>
                                            <Datepicker class="flex-1 mx-auto" v-model="exchangeDetail.agr_start_use" :disabled="checkDisableByStatus('consent')"></Datepicker>
                                        </td>
                                    </tr>
                                    <tr class="text-center">
                                        <td>請求開始月</td>
                                        <td>
                                            <Datepicker class="flex-1 mx-auto" month-picker format="yyyy年MM月" v-model="exchangeDetail.arg_invoice_start" :disabled="checkDisableByStatus('consent')"></Datepicker>
                                        </td>
                                    </tr>
                                    <tr class="text-center">
                                        <td>シリアルNo.</td>
                        ...
                        <div class="basis-[60%] d-flex flex-column input-gap">
                            ...
                            <div class="d-flex">
                                ...
                                <textarea class="flex-1 form-control" v-model="exchangeDetail.arg_remarks" :disabled="checkDisableByStatus('patient')"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="gap-3 py-2 d-flex align-items-center">
            ...
            <div class="d-flex basis-[50%] justify-content-center">
                <button class="btn w-[9rem]" @click="complete()" :disabled="exchangeDetail.status != 14">完了登録</button>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        ...mapMutations(['setTransitReloadParent', 'showErrorModal', 'setIsOpenTransitDetail']),
        ...
        createReceiptText: async function () {
            ...
            let info = [
                // displayDate(this.exchangeDetail.plan_send_deli, '', 'MM月DD日'),
                // displayDate(this.exchangeDetail.real_send_deli, '', 'MM月DD日（ddd）'),
                'MM月DD日',
                'MM月DD日（月）',
        ...
        setStatusList: function () {
            ...
            switch (status) {
                ...
                case 8:
                    ...
                    break
                case 9:
                    this.status_list = this.filterStatus([5, 6, 7, 8, 9, 10])
                    break
                case 15:
        ...
        reloadShipment: async function () {
            await this.getExchange(this.exchangeDetail.id).then((res) => {
                if(res && res.code == 200) {
                    ...
                    // this.exchangeDetail.desHistory = {...formatBeforeRequest(this.exchangeDetail.desHistory), ...patientHistory_update}
